...
atc_template_test_header = '''  
  <test name="$name" group="openCARP-Bidomain">
    ...
    <call>
      <command>python3 $dirname/regression/$modulerelpath $argv $flavorargs --ID _test_$name --overwrite-behaviour overwrite --CARP-opts='+ -mat_type aijkokkos -vec_type kokkos'</command>
    </call>
    <validations>'''
