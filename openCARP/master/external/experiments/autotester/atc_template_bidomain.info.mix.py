...
atc_template_test_header = '''  
  <test name="$name" group="openCARP-Bidomain">
    ...
    <call>
      <command>python3 $dirname/regression/$modulerelpath $argv $flavorargs --ID _test_$name --overwrite-behaviour overwrite --CARP-opts='+ -log_view -log_view_memory -log_view_gpu_time -mat_type aijcusparse -vec_type standard'</command>
    </call>
    <validations>'''
